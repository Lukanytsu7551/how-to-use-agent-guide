...
2. 首屏标题使用“选择合适的 Agent，推进真实工作。”，说明站点帮助用户按任务选择产品。首屏右侧使用抽象的产品符号与任务节点组合，形成明确视觉焦点，但不使用照片、渐变或营销插画。
3. 在首屏之后增加一条横向价值带，简短说明“真实任务”“可复现”“问题可排查”“持续更新”四项站点价值。该区域使用线性图标和文字，不显示无依据的虚构数据指标。
4. 两个不对称但结构一致的产品入口并列出现：Codex Guide 与 WorkBuddy Guide。每个入口展示适用场景、五个学习栏目及进入按钮。
5. “从任务开始”将写代码、整理资料、自动化办公、研究与分析、排查问题作为场景入口；其下按“选择产品、完成第一个任务、沉淀工作流、持续排查与更新”呈现四段学习路径。
6. 底部以列表呈现近期 Codex 更新、近期 WorkBuddy 更新和参考手册入口。
...
`https://workbuddy.homes/` 作为 WorkBuddy Guide 的视觉与信息组织参考。吸收以下原则：

- 首屏以一个明确的产品主题与抽象符号组合建立记忆点，而不是普通文档页标题。
- 使用“价值带 + 阅读路径”把文档目录转化为可执行的学习路线。
- 导航同时提供搜索、章节阅读与案例入口，减少用户在长教程中的迷失感。
- 图标采用同一线性几何语言，配合短标题和辅助说明，提升快速扫描效率。

不复制其荧光黄绿色、像素风图标、具体版式、文案、插画或源代码。新版网站以 Codex 蓝紫、WorkBuddy 薄荷绿及中性企业级底色为自己的识别体系，且需要在同一框架内平等展示两个产品。
